...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        {
            return port.sysidcurrent != 0 && port.compidcurrent == 1;
            //i.e compidcurrent == 1 is surface boat sysidcurrent=0 is initial port
        }

        private readonly Dictionary<MAVLinkInterface, GMapRoute> vehicleRoutes = new Dictionary<MAVLinkInterface, GMapRoute>();

        Color[] colors4trail ={
            Color.FromArgb(128,255,128), // green path
            Color.FromArgb(255,153,255),//violet path
            Color.FromArgb(204, 204, 0),//yellow path
            Color.FromArgb(153, 153, 255),//blue path
            Color.FromArgb(255, 210, 128),//orange path
            Color.FromArgb(255, 153, 153) };//red path
        private GMapRoute GetRoute(MAVLinkInterface port)
        {
            if (!vehicleRoutes.TryGetValue(port, out var route))
            {
        ...
        public FlightData()
        {
            ...
            //this.lbl_Mode.DataBindings.Add(new Binding("Text",this.bindingSourceHud,"mode",true,DataSourceUpdateMode.OnPropertyChanged,"","{0} mode"));

            //var binding_mode = new Binding("Text", this.bindingSourceHud, "mode", true);
            var binding_mode = new Binding("Text", bindingSource1, "mode", true, DataSourceUpdateMode.OnPropertyChanged);
            binding_mode.Format += (sender, e) =>
            {                
                if (e.Value.ToString()=="Auto")
                {
                    lbl_Mode.BackColor = Color.FromArgb(255, 182, 0); //near yellow
                }
                else
                {                 
        ...
        {
            GetRoute(MainV2.comPort).Points.Clear();
                        ...
                        tracklast.AddSeconds(2) < DateTime.Now)
                    {
                        ...
                        if (route == null)
                        {
                            ...
                            foreach (var port in MainV2.Comports)
                            {
                                ...
                                var route1 = GetRoute(port);

                                var currentloc1 = new PointLatLng(port.MAV.cs.lat, port.MAV.cs.lng);

                                route1.Points.Add(currentloc1);
                        ...
                        {
                            if (port.MAV.sysid <= 0)
                                continue;//when no vehicle connected its running so we guarded it
                            var route = GetRoute(port);

                            if (route.Points.Count > numTrackLength)
                            {
                                route.Points.RemoveRange(
                                    0,
                                    route.Points.Count - numTrackLength);
                            }
                        }
                        ...
                        // add new route point
                        // commented orginal code under 24july2026_DrawAllPathsFlightData start
                        //if (MainV2.comPort.MAV.cs.lat != 0 && MainV2.comPort.MAV.cs.lng != 0)
                        //{
                        //    route.Points.Add(currentloc);
                        //}
                        // commented orginal code under 24july2026_DrawAllPathsFlightData end
                        ...
                        foreach (var port in MainV2.Comports)
                        {
                            ...
                            var vehicleRoute = GetRoute(port);

                            var currentloc2 = new PointLatLng(
                                port.MAV.cs.lat,
                                port.MAV.cs.lng);

                            if (port.MAV.cs.lat != 0 && port.MAV.cs.lng != 0)
                            {
                                vehicleRoute.Points.Add(currentloc2);
                            }
                        }
                        // 24july2026_DrawAllPathsFlightData end
        ...
        //public int mav_batp_custom_number=0;
        ...
        private void tabPage1_Resize(object sender, EventArgs e)
        {
            ...
            G_RPM.Width = G_batp.Width;

            G_fuel.Location = new Point(Galt.Location.X + G_batp.Width, Galt.Location.Y + G_batp.Height); //alt gauge[1,1]
            G_fuel.Height = G_batp.Height;
        ...
        private void PopoutGaugeCloned_Circular( bool currentState, Action<bool> setState, Control gauge, string title,  Size size)
        {
            ...
            void StartDrag(Point screenPos)
            {
                dragging = true;
                dragStart = screenPos;
            }

            void DoDrag(Point screenPos)
            {
                if (!dragging)
                    return;

                popup.Left += screenPos.X - dragStart.X;
                popup.Top += screenPos.Y - dragStart.Y;
            ...
            popup.DoubleClick += (s, e) =>
            {
                popup.Close();
            };

            clonedGauge.DoubleClick += (s, e) =>
            {
                popup.Close();
            };
            ...
            popup.MouseWheel += (s, e) =>
            {
                wheelHandler(s, e);
            };

            clonedGauge.MouseWheel += (s, e) =>
            {
                wheelHandler(s, e);
            };

            // ensure wheel works immediately
            popup.MouseEnter += (s, e) => popup.Focus();
            clonedGauge.MouseEnter += (s, e) => popup.Focus();
        ...
        private AGaugeApp.AGauge CloneAGauge(AGaugeApp.AGauge source)
        {
            ...
            clone.CapsFonts = (Font[])source.CapsFonts.Clone(); // 08july2026_task1
            clone.CapFont = source.CapFont; // 08july2026_task1
